...
{
    public sealed class SubstringBeforeFunction : IBuiltinFunction
    {
        private const string ERROR_CODE_ARG_BAD_TYPE = "S0206"; // Argument must be a string
        ...
        {
            string functionName = "$substringBefore";
            JsonNode? strNode;
            JsonNode? charsNode;
            ...
            {
                return new Sequence(JsonValue.Create("")); // Return empty string
            }

            int index = sourceString.IndexOf(charsToFind);
            if (index == -1) // Not found
            {
                return new Sequence(JsonValue.Create("")); // Return empty string (JSONata spec for $substringBefore if not found is empty string)
            }

            return new Sequence(JsonValue.Create(sourceString.Substring(0, index)));
        }
    }
}
